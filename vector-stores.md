...
Cosine Similarity:
* This metric measures the angle between two vectors. The smaller the angle, the closer the vectors
...
Product Quantization works well for large datasets and high-dimension spaces. It can greatly speed up the nearest neighbor search and reduce the overall memory footprint by ~97%. The downside of using this compression technique is that it can lead to lower accuracy and recall. {cite}`vector-quantization`
